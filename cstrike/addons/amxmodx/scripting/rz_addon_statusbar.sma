...
public plugin_init()
{
	register_plugin("[ReZP] Addon: Status Bar (Enemy HP Fix)", REZP_VERSION_STR, "fl0wer");
...
@MSG_StatusValue(id, dest, player)
{
	...
	if (is_user_alive(player))
	{
		...
		if (targetType == SBAR_TARGETTYPE_TEAMMATE)
		{
			...
			// ИСПРАВЛЕНО: Выводим точное количество HP союзника вместо сломанных процентов
			add_formatex("^n[%l: %d HP]", "RZ_HEALTH", floatround(get_entvar(target, var_health)));

			if (rz_main_ammopacks_enabled())
				add_formatex(" [%l: %d]", "RZ_AMMOPACKS", get_member(target, m_iAccount));
		}
		// ФИКС ВРАГОВ: Если навели на противника и разрешено в кварах
		else if (mp_playerid != PLAYERID_MODE_TEAMONLY)
		{
			...
			// ИСПРАВЛЕНО: Выводим точное количество HP врага (зомби или босса) вместо сломанных процентов
			add_formatex("^n[%l: %d HP]", "RZ_HEALTH", floatround(get_entvar(target, var_health)));
	...
	// Если мы мертвы и наблюдаем за кем-то
	else if (get_entvar(player, var_iuser1) != OBS_NONE)
	{
		...
		// ИСПРАВЛЕНО: Выводим точное количество HP в режиме наблюдения
		add_formatex("^n[%l: %d HP]", "RZ_HEALTH", floatround(get_entvar(target, var_health)));
